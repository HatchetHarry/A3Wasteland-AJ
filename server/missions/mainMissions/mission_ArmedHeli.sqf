...
if((!hasInterface && !isServer) || isServer) then
{
	...
	_result = 0;
	_missionMarkerName = "ArmedHeli_Marker" + str time + str (random 1000000);
...
//aj - mod - Removed Gunships, added Hellcat
//	_vehicleClass = ["B_Heli_Light_01_armed_F", "B_Heli_Transport_01_F", "B_Heli_Attack_01_F", "O_Heli_Light_02_F", "O_Heli_Attack_02_F"] call BIS_fnc_selectRandom;
	_vehicleClass = ["B_Heli_Light_01_armed_F", "B_Heli_Transport_01_F",  "O_Heli_Light_02_F",  "I_Heli_light_03_F"] call BIS_fnc_selectRandom;
	...
	switch (true) do
	{
		case (_vehicleClass isKindOf "B_Heli_Light_01_armed_F"):        { _vehAmmo = 0.5 }; //pawnee
		case (_vehicleClass isKindOf "B_Heli_Transport_01_F"):    { _vehAmmo = 0.5 }; //ghosthawk
		case (_vehicleClass isKindOf "B_Heli_Attack_01_F"):       { _vehAmmo = 0.2 }; //ah-99
		case (_vehicleClass isKindOf "O_Heli_Light_02_F"):        { _vehAmmo = 0.5 };	//orca
		case (_vehicleClass isKindOf "O_Heli_Attack_02_F"):       { _vehAmmo = 0.8 }; //kaiman
		case (_vehicleClass isKindOf "I_Heli_light_03_F"):       { _vehAmmo = 0.5 }; //hellcat
	};
	...
	switch (true) do
	{
		case (_vehicle isKindOf "Heli_Attack_01_base_F"):
		{
			...
			_vehicle addMagazineTurret ["4Rnd_AAA_missiles", [0]];
		};
		case (_vehicle isKindOf "Heli_Attack_02_base_F"):
		{
			...
			_vehicle addMagazineTurret ["8Rnd_LG_scalpel", [0]];
		};
	};
